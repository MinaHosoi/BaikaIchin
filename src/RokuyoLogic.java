...
public class RokuyoLogic {
	...
	static public final String TIME_PATTERN ="HH:mm";


	 static String parseDateToString(Date date){
		String  str;
		if(date == null) {
			str = null;
		} else {
	 ...
	 static String parseTimeToString(Date date){
		String  str;
		if(date == null) {
			str = null;
		} else {
			str = new SimpleDateFormat(TIME_PATTERN).format(date);
		}
		return str;
	}


	static String RokuYo(int yyyy, int mm, int dd) {
		year = yyyy;
		month = mm;
		day = dd;
		...
		return "旧暦: " + kyureki[0] + "." + ((kyureki[2] < 10) ? "0" : "") + kyureki[2] + "."
				+ ((kyureki[3] < 10) ? "0" : "") + kyureki[3] + " " + ROKUYO[(kyureki[2] + kyureki[3] - 2) % 6];
	}
